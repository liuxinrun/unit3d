...
class UniDet3D(Base3DDetector):
    ...
    def trim_bboxes_by_superpoints_vis_attn(self, sp_pts_mask, point, 
                                   bboxes, labels, scores):
        """Trim bounding boxes based on superpoint masks.
    ...
    def _single_scene_multiclass_nms_vis_attn(self, bboxes, scores, 
                                     labels,attn_weights, fast_nms, iou_thr):
        """Multi-class nms for a single scene.
        ...
        classes = labels.unique()
        with_yaw = bboxes.shape[1] == 7
        nms_bboxes, nms_scores, nms_labels,nms_attn_w = [], [], [], []
        for class_id in classes:
            ids = scores[labels == class_id] > self.test_cfg.score_thr
            if not ids.any():
                continue
            ...
            class_bboxes = bboxes[labels == class_id][ids]
            class_labels = labels[labels == class_id][ids]
            class_attn = attn_weights[labels == class_id][ids]
            if with_yaw:
                nms_ids = nms3d(class_bboxes, class_scores, iou_thr)
            else:
                if fast_nms:
            ...
            nms_scores.append(class_scores[nms_ids])
            nms_labels.append(class_labels[nms_ids])
            nms_attn_w.append(class_attn[nms_ids])

        if len(nms_bboxes):
            nms_bboxes = torch.cat(nms_bboxes, dim=0)
            nms_scores = torch.cat(nms_scores, dim=0)
            nms_labels = torch.cat(nms_labels, dim=0)
            nms_attn_w = torch.cat(nms_attn_w, dim=0)
        else:
            nms_bboxes = bboxes.new_zeros((0, bboxes.shape[1]))
            nms_scores = bboxes.new_zeros((0, ))
            nms_labels = bboxes.new_zeros((0, ))
            nms_attn_w = nms_attn_w.new_zeros((0, nms_attn_w.shape[1]))

        return nms_bboxes, nms_scores, nms_labels, nms_attn_w
