...
def axis_aligned_mpdiou_loss(pred: Tensor, target: Tensor) -> Tensor:
    """Calculate the MPDIoU loss (1-MPDIoU) of two sets of axis aligned bounding
    boxes in 3D. Note that predictions and targets are one-to-one corresponded.
    ...
    Returns:
        torch.Tensor: MPDIoU loss between predictions and targets.
    """
    # Compute axis-aligned IoU
    axis_aligned_iou = AxisAlignedBboxOverlaps3D()(pred, target, is_aligned=True)
    iou_loss = 1 - axis_aligned_iou

    # Split corner coordinates
    xp1, yp1, zp1, xp2, yp2, zp2 = pred.split(1, dim=-1)
    xt1, yt1, zt1, xt2, yt2, zt2 = target.split(1, dim=-1)

    # Squared distances between corresponding corners
    d1_sq = (xp1 - xt1)**2 + (yp1 - yt1)**2 + (zp1 - zt1)**2
    d2_sq = (xp2 - xt2)**2 + (yp2 - yt2)**2 + (zp2 - zt2)**2
    ...
    z_min = torch.minimum(zp1, zt1)
    z_max = torch.maximum(zp2, zt2)
    c2 = (x_max - x_min)**2 + (y_max - y_min)**2 + (z_max - z_min)**2

    # Penalty term: normalized sum of corner distances
    penalty = (d1_sq + d2_sq) / c2
    penalty = penalty.squeeze(-1)

    # Final MPDIoU loss
    mpdiou_loss = iou_loss + penalty
    return mpdiou_loss


@MODELS.register_module()
class UniDet3DAxisAlignedIoULoss(nn.Module):
    ...
    def __init__(self,
        ...
        super(UniDet3DAxisAlignedIoULoss, self).__init__()
        assert mode in ['iou', 'diou','mpdiou']
        if mode == 'iou':
            self.loss = axis_aligned_iou_loss
        elif mode == 'diou':
            self.loss = axis_aligned_diou_loss
        else:
            self.loss = axis_aligned_mpdiou_loss
